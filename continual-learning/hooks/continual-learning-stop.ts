...
const FOLLOWUP_MESSAGE =
  `Run the \`continual-learning\` skill now in incremental hook-triggered mode. Use the \`agents-memory-updater\` subagent for the full memory update flow. Detect the current host/runtime from hook data and environment. Prefer the hook-provided transcript path for the current parent transcript, and use host-specific transcript locations when broader discovery is needed. Use incremental transcript processing with index file \`${INCREMENTAL_INDEX_PATH}\`: only consider parent transcripts not in the index or transcripts whose mtime is newer than indexed mtime. Exclude subagent transcripts from parent-session counts. Have the subagent refresh index mtimes, remove entries for deleted transcripts, and update \`AGENTS.md\` only for high-signal recurring user corrections and durable workspace facts. Exclude one-off/transient details, stale missing-file references, and secrets. If no meaningful updates exist and the targeted parent transcripts are fully accounted for, respond exactly: No high-signal memory updates.`;
